...
public class CreateRestaurantCommandValidator : AbstractValidator<CreateRestaurantCommand>
{
    ...
    public CreateRestaurantCommandValidator()
    {
        ...
        //    if (!isValidCategory)
        //    {
        //        context.AddFailure("Category", "Invalid categor. Please choose from valid categories.");
        //    }
        //});

        RuleFor(dto => dto.ContactEmail).EmailAddress().WithMessage("Please provide a valid email address");
        RuleFor(dto => dto.PostalCode).Matches(@"^\d{2}-\d{3}$").WithMessage("Please provide a valid postal code in the format: XX-XXX");
    }
}
